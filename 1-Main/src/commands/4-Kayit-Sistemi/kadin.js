...
run: async (client, message, args, embed, prefix) => { 
    ...
    {
    message.react(red)
    message.lineReply(`\`${prefix}kadın <@Pusha/ID> <Isim> <Yaş>\``).then(x=>x.delete({timeout:5000})) 
    return }
    ...
    {
    message.react(red)
    message.lineReply(`Kayıt ettiğin üyenin yaşı 14'ten küçük olamaz.`).then(x => x.delete({timeout: 5000})); 
    return }
