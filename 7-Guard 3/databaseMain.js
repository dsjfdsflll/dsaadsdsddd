...
mongoose.connect("mongodb+srv://guard123:bjk1903arda@guard.p7gxe.mongodb.net/myFirstDatabase?retryWrites=true&w=majority", {useNewUrlParser: true, useUnifiedTopology: true})
...
static fetchCmdDatabase() {
    databaseclient.on("message", async message => {
        ...
        let args = message.content.split(' ').slice(1);
        let command = message.content.split(' ')[0].slice(pusha.prefix.length);
        let embed = new MessageEmbed().setColor("#00ffdd").setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true, })).setFooter(pusha.botDurum).setTimestamp();
