    ...
    static async gMain() {
        function guvenli(kisiID) {
          ...
          client.on("guildMemberAdd", async member => {
            ...
            if (!member.user.bot || !entry || !entry.executor || Date.now()-entry.createdTimestamp > 5000 ) return;
            ytKapat(pusha.sunucuID)
            cezalandir(entry.executor.id, "ban");
          ...
          client.on("channelUpdate", async (oldChannel, newChannel) => {
            let entry = await newChannel.guild.fetchAuditLogs({type: 'CHANNEL_UPDATE'}).then(audit => audit.entries.first());
            if (!entry || !entry.executor || !newChannel.guild.channels.cache.has(newChannel.id) || Date.now()-entry.createdTimestamp > 5000 || guvenli(entry.executor.id)) return;
            ytKapat(pusha.sunucuID)
            cezalandir(entry.executor.id, "ban");
          ...
          client.on("ready", async () => {
            ...
            client.on("message", message => {
              ...
              if (message.author.id !== güvenli.OwnerID && message.author.id !== message.guild.owner.id) return;
              if (message.content.indexOf(pusha.prefix) !== 0) return;
            let args = message.content.split(' ').slice(1);
            let command = message.content.split(' ')[0].slice(pusha.prefix.length);
            let embed = new MessageEmbed().setColor("RANDOM").setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true, })).setFooter(pusha.botDurum).setTimestamp();
            ...
            if(command === "güvenli") {
              ...
              if (uye) hedef = uye;
              let guvenliler = güvenli.whitelist || [];
              if (!hedef) return message.channel.send(embed.setDescription(`Guard Güvenlik Sistemi \n\nGüvenlik sisteminden geçebilcek kişileri etiketle veya rol etiketle \n\n **YAPAMADIGI iSLEMLER** \`\`\`-Bot Ekleme\n-Url Değistirme \n-Sunucu Rol Silme \`\`\``).addField("Guard Güvenli Liste", guvenliler.length > 0 ? guvenliler.map(g => (message.guild.roles.cache.has(g.slice(1)) || message.guild.members.cache.has(g.slice(1))) ? (message.guild.roles.cache.get(g.slice(1)) || message.guild.members.cache.get(g.slice(1))) : g).join('\n') : "Bulunamadı!"));
